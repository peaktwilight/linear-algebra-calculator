...
class CramersRule:
    ...
    def render_exercise_examples(self):
        """Render Week 22 exercise examples for Cramer's Rule."""
        st.subheader("Week 22 Exercise Examples - Cramer's Rule")
        
        example = st.selectbox(
            "Select Example:",
            ["12.20 Cramer's Rule (725615)", "12.22 Cramer's Rule (911705)"]
        )
        
        if example == "12.20 Cramer's Rule (725615)":
            st.write("### Example 12.20: Basic Cramer's Rule Application")
            ...
            if st.button("Solve Example 12.20"):
                self._solve_cramers_rule(A, b)
                
        elif example == "12.22 Cramer's Rule (911705)":
            st.write("### Example 12.22: 3×3 System")
            
            examples = [
                ("Part a", np.array([[1, 2, 3], [4, 5, 6], [7, 8, 10]]), np.array([6, 15, 25])),
                ("Part c", np.array([[2, -1, 3], [1, 1, -1], [3, 2, 1]]), np.array([8, 2, 13]))
            ]
            
            for name, A, b in examples:
                with st.expander(name):
                    st.write(f"**{name} System:**")
                        ...
                        for j in range(A.shape[1]):
                            ...
                            if j == 0:
                                if coeff == 1:
                                    ...
                                    terms.append(f"{coeff:.4g}x_{{{j+1}}}")
                            else:
                                if coeff == 1:
                                    terms.append(f"+ x_{j+1}")
                                elif coeff == -1:
                                    ...
                                    terms.append(f"{coeff:.4g}x_{{{j+1}}}")
                        eq_lines.append(" ".join(terms) + f" &= {b[i]:.4g}")
                    
                    equations = r"\begin{align}" + " \\\\ ".join(eq_lines) + r"\end{align}"
                    st.latex(equations)
